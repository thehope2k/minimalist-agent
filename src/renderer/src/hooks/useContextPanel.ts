...
export function useContextPanel({ sessionId, cwd, pinnedAssets }: UseContextPanelProps) {
  ...
  const [localPinned, setLocalPinned] = useState<Set<string>>(() => new Set(pinnedAssets ?? []));
  ...
  useEffect(() => {
    ...
    window.api.context
      .estimateTokens([...localPinned], cwd)
      .then(setTokenEstimate)
      .catch(() => {});
  }, [localPinned, cwd]);

  const pin = useCallback(
    async (scopedSlug: string) => {
      if (!sessionId) return;
  ...
  const isPinned = useCallback(
    (scope: ContextScope, slug: string): boolean => {
  ...
  const pinnedSkills = available.skills.filter((s) => localPinned.has(`${s.source}:${s.slug}`));

...
export function usePlanState(activeSessionId: string | null, deps: PlanStateStoreDeps) {
  ...
  const findLastAssistantTurnId = useCallback(
  ...
  const resolvePlanAnchorTurnId = useCallback(
    (sid: string): string | null => {
  ...
  const setSessionPlan = useCallback(
    (sid: string, plan: Plan | null, options?: { resetAnchor?: boolean }) => {
      if (!plan) {
  ...
  const getPlanForMessage = useCallback(
    (sid: string | null | undefined, messageId: string): Plan | null => {
      if (!sid) return null;
  ...
  useEffect(() => {
    ...
    const unsubPhaseUpdated = window.api.planning.onPhaseUpdated(
      (sid: string, planId: string, phase: Phase) => {
    ...
    const unsubError = window.api.planning.onPlanError(
      (sid: string, planId: string, error: string, phaseId?: string) => {
        const current = activePlanBySession.current.get(sid);
        if (!current || current.id !== planId) return;
        const finalAnchor = resolvePlanAnchorTurnId(sid);
        if (finalAnchor) {
    ...
    const unsubApprovalRequired = window.api.planning.onApprovalRequired(
      (sid: string, planId: string, phase: Phase) => {
        const current = activePlanBySession.current.get(sid);
        if (!current || current.id !== planId) return;

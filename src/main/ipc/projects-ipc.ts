...
export function registerProjectsIpc(): void {
  ...
  ipcMain.handle('projects:create', (_e, input: ProjectInput): Project => createProject(input));
  ipcMain.handle('projects:reorder', (_e, ids: string[]): void => reorderProjects(ids));
  ipcMain.handle(
    'projects:update',
    (_e, id: string, patch: Partial<Omit<Project, 'id' | 'createdAt'>>): Project | null =>
      updateProject(id, patch),
  );
  ipcMain.handle('projects:delete', (_e, id: string): { ok: boolean; sessionsCleared: number } => {

...
import {
  SKILLS_REFERENCE_MD,
  SKILLS_REFERENCE_VERSION,
} from './reference-doc';

const VERSION_MARKER = (v: string) => `<!-- skills-reference-version: ${v} -->`;
...
export function installSkillsReferenceDoc(): void {
  const dest = Paths.skillsReferenceDoc();
  const expectedMarker = VERSION_MARKER(SKILLS_REFERENCE_VERSION);
  ...
  // skillsReferenceDoc() goes through root() but doesn't mkdir, so make

...
| Assistant body (identity, capabilities, read-first, **skills with global+project scope + reference-doc pointer**, **extensions with correct paths + reference-doc pointer**, mermaid, math, rich blocks, interaction guidelines, git co-author, web search) | `getAssistantPrompt()`                                           | always                         |                                            ~1,850 |
...
but has **no section in the prompt**, so the model never emits it. Either add a short section (costs prompt budget — run
the [checklist](#6-checklist-before-you-touch-the-prompt))

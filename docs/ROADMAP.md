...
| Capability | Detail |
|---|---|
| **Voice dictation (Cmd+Shift+M)** | On-device speech-to-text in the composer via `sherpa-onnx-node` (Silero VAD + Moonshine tiny-en, int8, CPU inference). Streaming: speech is detected and transcribed in segments while recording, not just once at the end; each segment inserts at the current cursor position. Model set (~tens of MB) downloads once from `k2-fsa/sherpa-onnx` GitHub releases on first use, SHA256-verified, cached under `userData/voice-models/`; fully offline afterward — audio never leaves the machine. Mic permission scoped to the app-shell origin only (`installMediaPermissions()`), not Electron's permissive default. English-only. See [VOICE.md](VOICE.md) |
